...
class FirstStageModel:
    def __init__(self):
        ...
        self.__send_fwd_outputs = Global.tf_lib().send_tensor(
        ...
        with tf.control_dependencies([self.__send_fwd_outputs]):
            self.__send_fwd_labels = Global.tf_lib().send_tensor(
                self.__send_fwd_labels_placeholder, receiver=next_stage_rank,
                name='0-forward-label-to-1')

        self.__receive_diff_placeholder = tf.compat.v1.placeholder(
            dtype=tf.float32, shape=(None, 196))
        self.__receive_diff = Global.tf_lib().receive_tensor(
            self.__receive_diff_placeholder, sender=next_stage_rank)
    ...
    def __send_forward_propagation_result(self, begin: int, end: int):
        ...
        if executing_eagerly():
            Global.tf_lib().send_tensor(
                tf.constant(outputs, dtype=tf.float32),
                receiver=next_stage_rank,
                name='0-forward-input-to-1'
            )
            Global.tf_lib().send_tensor(
                tf.constant(labels, dtype=tf.float32), receiver=next_stage_rank,
                name='0-forward-label-to-1'
            )
        else:
            self.__session.run(
    ...
    def __wait_back_propagation_msg(self):
        ...
        msg = Global.listen_message()

        msg_obj = json.loads(msg.msg)
        log(f'got msg_obj: {msg_obj}')
        if msg_obj['type'] == MSG_DONE:
            ...
            if executing_eagerly():
                diff = Global.tf_lib().receive_tensor(tf.zeros(shape=(
